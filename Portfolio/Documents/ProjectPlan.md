# DISP Group 4 - Project Plan

## Overview
This document outlines the tasks, assignments, and progress tracking for the DISP Group 4 project.

---

## Task Table

| Task # | Description                        | Assigned To     | Completed Date | Duration | Status         |
| ------ | ---------------------------------- | --------------- | -------------- | -------- | -------------- |
| 1      | SD Model                           | Nathan & Wilson | 17/11/2025     | 13 hr    | ✅ Complete     |
| 2      | SR Model                           | Raees, Nathan & Wilson  |                |          | 🔄 In Progress |
| 3      | Strategic business process model   |  Nick & Sam               |                |          | ⬜ Incomplete   |
| 4      | Operational business process model | Nick, Nathan & Wilson      |                |          | 🔄 In Progress |
| 5      | Setup ReadMe file                  | Nathan                |                |          | ⬜ Incomplete   |
| 6      | Setup Project initial Plan         | Nick & Sam      | 17/11/2025     | 0.5 hr   | ✅ Complete     |
| 7      | Setup Camunda Repo and Link to Git | Nick            | 17/11/2025     | 0.5hr    | ✅ Complete     |
| 8      |                                    |                 |                |          | ⬜ Incomplete   |
| 9      |                                    |                 |                |          | ⬜ Incomplete   |
| 10     |                                    |                 |                |          | ⬜ Incomplete   |

---

## Status Legend
- ✅ Complete
- ⬜ Incomplete
- 🔄 In Progress

---

## Notes
- Update **Assigned To** with team member names.
- Fill in **Completed Date** and **Duration** once tasks are finished.
- Add more rows as needed for additional tasks.

---
## Team Members
- Nick
- Sam
- Wilson
- Nathan
- Raees
